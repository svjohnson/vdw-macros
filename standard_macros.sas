...
%macro GetFollowUpTime(People    /* Dset of MRNs */
   ...
   %put ============================================================== ;
   %put ;
   %put Macro GetFollowUpTime V0.91: ;
   %put ;
   %put Creating a dset "&OutSet", which will look just like "&People" except ;
   %put that it will have an additional variable "&CallEndDateVar", which will ;
   %put hold the earliest of date-of-last-enrollment, or &EndDate (or, if the ;
   ...
   proc sql noprint ;
      ...
      from  &People as p INNER JOIN
            &EnrollDset as e
      on    p.MRN = e.MRN
      where intnx('day', &IndexDate, -&GapTolerance, 'same') le e.enr_end AND
            intnx('day', &EndDate  ,  &GapTolerance, 'same') ge e.enr_start
      order by mrn, enr_start
      ;
   quit ;
   ...
   proc sql noprint ;
      ** How many days long is the period of interest? ;
      ...
      select ("&EndDt"d - "&StartDt"d + 1) as TotDays
            , intnx('day', "&StartDt"d, -&DaysTol, 'same') as earliest_start format = date9.
            , intnx('day', "&EndDt"d  ,  &DaysTol, 'same') as latest_end     format = date9.
               into :TotDays, :EarliestStart, :LatestEnd
      from  dual ;
   quit ;
...
/*
  %put Earliest start = &EarliestStart and Latest End = &LatestEnd  ;
  %put Earliest start = &EarliestStart and Latest End = &LatestEnd  ;
  %put Earliest start = &EarliestStart and Latest End = &LatestEnd  ;
  %put Earliest start = &EarliestStart and Latest End = &LatestEnd  ;
  %put Earliest start = &EarliestStart and Latest End = &LatestEnd  ;
   ...
   * Calculate # of days between start & end date. ;
   proc sql ;
      ...
      select mrn
            ...
            , (sum(days) = &TotDays) as ContinuouslyEnrolled label = "0/1 flag answering was this person continuously enrolled from &StartDt to &EndDt. (disregarding gaps up to &DaysTol days)?"
      from _period_days
      group by mrn
      ;
