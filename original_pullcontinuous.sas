...
* PRETIME    * CONTINUOUS MONTHS OF MEMBERSHIP PRIOR TO INDEX DT REQUIRED      ;
*               (DEFAULT=0)                                                    ;
* PREGAP     * NUMBER OF MONTHS DISENROLLMENT ALLOWED PRIOR TO INDEX DT        ;
*                (DEFAULT=0 CUMULATIVE)                                        ;
* POSTIME    * CONTINUOUS MONTHS OF MEMBERSHIP AFTER INDEX DT REQUIRED         ;
*                (DEFAULT=0)                                                   ;
* POSTGAP    * NUMBER OF MONTHS DISENROLLMENT ALLOWED AFTER INDEX DT           ;
*                (DEFAULT=0 CUMULATIVE)                                        ;
*                                                                              ;
...
TITLE "TO PULL SAMPLE WITH &PRETIME CONTINUOUS MONTHS MEMBERSHIP PRE DX DATE" ;
TITLE2 "(&PREGAP MONTH GAP) &POSTIME CONTINUOUS MONTHS MEMBERSHIP POST DX DATE";
TITLE3 "(&POSTGAP MONTH GAP)";;
DATA DATEX;
     SET &PLUGIN;
...
libname enrol "&_EnrollLib." access=readonly;
PROC SQL;
     ...
     SELECT  DATEX.POSTDTMON, DATEX.POSTDTYR, DATEX.PREDTYR, DATEX.PREDTMON,
          ...
          1 AS ANCHOR
     FROM DATEX, enrol.&_EnrollData AS ENR
...
DO I = MATRXPRE TO MATRXPOST ;
  ...
  END;
/*PUT MRN 'ENRX(I)=' ENRX(I) 'I=' I 'MATRXIND=' MATRXIND 'KTRPRE=' KTRPRE
     'GAPPRE=' GAPPRE 'GAPPOST=' GAPPOST;*/
END;
IF (KTRPRE >= (&PRETIME-&PREGAP) AND KTRPOST >= (&POSTIME-&POSTGAP))
  THEN OUTPUT SELECTED;
ELSE OUTPUT EXCLUDED;
RUN;

TITLE "MEMBERS WITH &PRETIME CONTINUOUS MONTHS MEMBERSHIP PRE DX DATE";
TITLE2 "(&PREGAP MONTH GAP) &POSTIME CONTINUOUS MONTHS MEMBERSHIP POST";
TITLE3 "DX DATE (&POSTGAP MONTH GAP)";;
...
Proc print data= SELECTED (obs=30);
     VAR E1-E&MONMAX INDEXDT MATRXPRE MATRXIND MATRXPOST
           KTRPRE GAPPRE KTRPOST GAPPOST;
     run;
