  	    ...
  	    create table &OutSet as
	  			select r.*
   				...
   				where r.RxDate BETWEEN "&StartDt"d AND "&EndDt"d AND
         				r.NDC in (select NDC from &DrugLst) ;
      quit ;
      libname __rx clear ;
   %end ;
...
%macro BreastCancerDefinition01(StartDt = 01Jan1997
   ...
   */
   ...
   proc sql number ;
      ...
      select *
      from  _FirstBTs
      where MRN in (select MRN from _FirstBTs group by MRN having count(*) > 1);
      title ;
...
* StudyID, creating a crosswalk dset that relate the
...
%macro PullContinuous2(
          InSet                   /*Name of the input dataset of MRNs of the ppl
                                      whose enrollment you want to check*/
        , OutSet                  /*Name of the output dataset of only the
                                      continuously enrolled people. */
        , IndexDate               /*Either the name of a date variable in InSet,
                                      or a complete date literal
                                      (e.g., "01Jan2005"d) */
        , PreIndexEnrolledMonths  /*# of months of enrollment required prior to
                                      the index date. */
        , PreIndexGapTolerance    /*Length of enrollment gaps you consider to be
                                      ignorable for pre-index date enrollment.*/
        , PostIndexEnrolledMonths /*# of months of enrollment required post
                                      index date. */
        , PostIndexGapTolerance   /*Length of enrollment gaps you consider to be
                                      ignorable for post-index date enrollment*/
        , DebugOut
        , EnrollDset = __enroll.&_EnrollData /* For testing. */
        ) ;
   ...
   proc sql feedback ;
      * Create a dset to hold the output data. ;
      create table &OutSet
      like &InSet ;


      %* We only need our peoples enroll recs over the period from
         (IndexDate - (PreIndexEnrolledMonths + PreIndexGapTolerance));
 ...
 %let PostMonthCount = %eval(&PostIndexEnrolledMonths + &PostIndexGapTolerance);
      ...
      * This statement is only conditioned on ENR_Year b/c I want to be able ;
      * to use the index on that var if possible. ;
      create table _grist as
      select e.MRN
            , mdy(e.Enr_Month, 1, e.Enr_Year) as EnrollDate format = mmddyy10.
            , &IndexDate                      as IDate      format = mmddyy10.
      from  &EnrollDset as e INNER JOIN
            &InSet as i
      on    e.MRN = i.MRN
      WHERE ENR_Year BETWEEN year(intnx("MONTH", &IndexDate, -&PreMonthCount))
                         AND year(intnx("MONTH", &IndexDate, &PostMonthCount))
      ;

      * Since the above is only conditioned on ENR_Year, we need to now get rid;
      * of extraneous records that fall outside the range ;
      delete from _grist
      where EnrollDate not BETWEEN intnx("MONTH", IDate, -&PreMonthCount) AND
                                   intnx("MONTH", IDate, &PostMonthCount)
      ;

      * Anybody enrolled for a total of &PreIndexEnrolledMonths + ;
      * &PostIndexEnrolledMonths months was definitely continuously enrolled, ;
      * and definitely belongs in the output dset. ;
      create table _continuously_enrolled as
      select MRN
      from _grist
      group by MRN
      having count(*) ge (&PreMonthCount + &PostMonthCount + 1)
      ;

      %put ;
      %put ;
      %put &SQLOBS people were continuously enrolled during the pre/post ;
      %put   index date period of interest. ;
      %put ;
      %put ;


      * Move their data into outset. ;
      insert into &OutSet
      select *
      from &InSet
      where MRN in (select MRN from _continuously_enrolled) ;

      * Remove  continuously-enrolled people from _grist, for efficiencys sake;
      delete from _grist
      where MRN in (select MRN from _continuously_enrolled) ;

      drop table _continuously_enrolled ;

   quit ;

   * Who has a gap longer than the tolerance? ;
   proc sort data = _grist ;
      by MRN EnrollDate ;
   run ;

   data &DebugOut _insufficiently_enrolled ;
      retain _LastDate . ;
      set _grist ;
      by MRN EnrollDate ;
      ...
      * EnrollDate, and the one on the last rec. We always expect a 1-month gap;
      * so we subtract out the expected gap. ;
      ThisGap = intck("MONTH", _LastDate, EnrollDate) - 1 ;
      EndGap = 0 ;


      * Two rec types are special--firsts and lasts w/in an MRN group. ;
      if first.MRN
        then ThisGap = intck("MONTH"
                           , intnx("MONTH", IDate, -&PreIndexEnrolledMonths)
                           , EnrollDate) ;
      if last.MRN
        then EndGap  = intck("MONTH"
                           , EnrollDate
                           , intnx("MONTH", IDate, &PostIndexEnrolledMonths)) ;

      * Ignore the smaller of the two gaps. ;
      MaxGap = max(ThisGap, EndGap) ;

      if MaxGap gt 0 then do ;
         * Weve got a gap--see if it is pre or post index date, and whether it ;
         * exceeds the appropriate tolerance. ;
         if EnrollDate le IDate then do ;
            * The gap ends before the index date--eval just the pre-gap ;
            if MaxGap gt (&PreIndexGapTolerance)
               then output _insufficiently_enrolled ;
         end ;
         else do ;
            * Its a post-index gap ;
            * Does the gap cross the index date? ;
            if (_LastDate ne .) and (_LastDate lt IDate) then do ;
               * Straddle--eval both pre and post gaps ;
               /*
                  Note that by splitting up straddle-gaps
                  into pre- and post- gaps we are implicitly
                  giving everybody credit for having been
                  enrolled on the index date (in the index
                  month).  This seems preferable to
                  arbitrarily choosing to evaluate it as a
                  pre or post gap.
               */
               * ThisGap = <<diff between LastDate and IDate>> ;
               * EndGap = <<diff between IDate and EnrollDate>> ;
               ThisGap = intck("MONTH", _LastDate, IDate) - 1  ;
               EndGap = intck("MONTH", IDate, EnrollDate) - 1 ;
               if ThisGap gt (&PreIndexGapTolerance)
                 or EndGap gt (&PostIndexGapTolerance)
                 then output _insufficiently_enrolled ;
            end ;
            else do ;
               * No straddle-evaluate just against the post-index gap tolerance;
               if MaxGap gt (&PostIndexGapTolerance)
                 then output _insufficiently_enrolled ;
            end ;
         end ;
      end ;
      %if %length(&DebugOut) > 0 %then %do ;
         output &DebugOut ;
      %end ;
   ...
   proc sql ;
      * Remove the insufficiently-enrolled people from _grist, so that dset has;
      *  only the IDs of people who passed the gap testing. ;
      delete from _grist
      where MRN in (select MRN from _insufficiently_enrolled) ;

      insert into &OutSet
      select *
      from  &InSet
      where MRN in (select MRN from _grist) ;

      drop table _grist ;
      drop table _insufficiently_enrolled ;
   quit ;

   libname __enroll clear ;
%mend PullContinuous2 ;
...
%macro GetVarList(DSet         /* Name of the dset you want collapsed. */
                , PersonID     /* Name of the var that contains a unique person identifier. */
...
%macro CollapsePeriods2(Lib          /* Name of the library containing the dset you want collapsed */
